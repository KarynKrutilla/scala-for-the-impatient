...
object Exercise08 {
  def main(args:Array[String]) {
    def dropAllButFirstNegative(arr:ArrayBuffer[Int]) = {
      ...
      for(i <- 0 until negatives.size) arr.remove(negatives(i)-i)
      arr
    }
    
    // Test
    val withoutFirstNegative = dropAllButFirstNegative(ArrayBuffer(1, -2, 3, -4, 5, -6, 7))
    for(i <- 0 until withoutFirstNegative.size) println(withoutFirstNegative(i))
    assert(withoutFirstNegative == ArrayBuffer(1,-2,3,5,7))
  }
}